...
function setup() {
	...
	createP("Flocking with Reynolds' three rules - separation (avoid), alignment (copy), cohesion (centre) (");
	createP("sketch12.js");
}
...
function Thing(startX,startY) { // thing constructor
	...
	this.stayTogether = function(thingArray) {
		closeSum = createVector();
		closeCount = 0;
		for (var i = 0; i < thingArray.length; i++) {
			locSep = p5.Vector.dist(this.pos,thingArray[i].pos);
			if (locSep>0 && locSep<this.radar) {
				closeSum.add(thingArray[i].pos);	
				closeCount++	
			}
		}	
		if (closeCount>0) {
			...
			closeSum.normalize();
			closeSum.mult(this.maxSpeed);
			this.steering = p5.Vector.sub(closeSum,this.loc);
			this.steering.limit(this.maxForce);
		}	else {
			this.steering.mult(0);
		} // else
