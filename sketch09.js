...
function Thing(startX,startY) { // thing constructor
	...
	this.separate = function(thingArray) {
		this.desSep = this.d*1.1// desired separation in body widths
		closeSum = createVector();
		closeCount = 0;
		for (var i = 0; i < gThings.length; i++) {
			locSep = p5.Vector.dist(this.pos,gThings[i].pos);
//			print(sepLocal); 
			if (locSep>0 && locSep<this.desSep){
				diffVect = p5.Vector.sub(this.pos,gThings[i].pos);
				diffVect.normalize();
				diffVect.div(locSep);
